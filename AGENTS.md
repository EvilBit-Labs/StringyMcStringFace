...
| `--only-tags` |       | `Vec<Tag>`            | Repeatable, `value_enum` (Tag derives `ValueEnum`)                                                                                                                             |
| `--no-tags`   |       | `Vec<Tag>`            | Repeatable, runtime overlap check against the resolved include set (`--only-tags` plus the convenience flags)                                                                  |
| `--imports`   |       | bool                  | Shorthand for `--only-tags import`; conflicts with `--only-tags`/`--raw`, compatible with `--no-tags` unless the excluded tag is in the include set                            |
| `--exports`   |       | bool                  | Shorthand for `--only-tags export`; conflicts with `--only-tags`/`--raw`, compatible with `--no-tags` unless the excluded tag is in the include set                            |
| `--symbols`   |       | bool                  | Shorthand for `--only-tags demangled` (DemangledSymbol only); conflicts with `--only-tags`/`--raw`, compatible with `--no-tags` unless the excluded tag is in the include set  |
