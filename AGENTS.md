...
**CRITICAL**: Never use emojis, em-dashes (—), or other non-Latin characters in code or documentation.

- Use standard ASCII hyphens (`-`) instead of em-dashes (—) or en-dashes (–)
- Use standard ASCII quotes (`"` or `'`) instead of smart quotes (" " ' ')
- Use standard ASCII apostrophes (`'`) instead of curly apostrophes (')
- Avoid Unicode symbols and emojis entirely
- Use text descriptions instead of symbolic indicators
