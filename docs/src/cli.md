...
| `--no-tags TAG`   | Exclude strings with any of these tags; repeatable. A tag also in the resolved include set fails validation                 | none    |
| `--imports`       | Shorthand for `--only-tags import`; conflicts with `--only-tags`/`--raw`, compatible with `--no-tags` (fails on overlap)    | all     |
| `--exports`       | Shorthand for `--only-tags export`; conflicts with `--only-tags`/`--raw`, compatible with `--no-tags` (fails on overlap)    | all     |
| `--symbols`       | Shorthand for `--only-tags demangled`; conflicts with `--only-tags`/`--raw`, compatible with `--no-tags` (fails on overlap) | all     |
